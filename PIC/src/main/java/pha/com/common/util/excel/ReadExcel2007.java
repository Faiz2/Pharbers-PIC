package pha.com.common.util.excel;
...
import pha.com.common.util.ReflectUtil;
						...
						if(switchbtn ==false && hasTop==false){
							if(!rowlist.get(i).toString().trim().equals(title[i]) && rowlist.get(i).toString().indexOf(title[i]) == -1){
								...
								try {
									...
									System.err.println(rowlist.get(i).toString()+":该标题有误或顺序错乱请重新检查模板！");
									ReflectUtil.invokeSuperSetter(target, "exceptionString", rowlist.get(i).toString()+":该标题有误或顺序错乱请重新检查模板");
									flag = true;
									//stop();
								} catch (Exception e) {
							...
							try {
								...
								ReflectUtil.invokeSetter(target, fieldNames[i], rowlist.get(i));
							} catch (Exception e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
								flag = true;
