...
import pha.com.common.base.util.ReflectUtils;
/**
 * <h2>分页拦截器</h2>
 * 
 * <p>
...
public class PageInterceptor implements Interceptor {
	...
	private String getPageSql(Page<?> page, String sql) {
		if ("mysql".equals(databaseType)) {
			...
			return getPageSqlOfSqlServer(page,sql);
		}
		return null;
	}
	 ...
	 */
	private String getPageSqlOfSqlServer(Page<?> page, String sql){
		int a = page.getPageIndex();
		int b = page.getPageSize();
		// 计算第一条记录的位置，Oracle分页是通过ROWNUM进行的，而ROWNUM是从1开始的
		int offset = (a - 1) * b + 1;
		String newSql = sql;
		String parameter = newSql.substring(6, newSql.indexOf(",")==-1?newSql.indexOf("from"):newSql.indexOf(","));
		if(newSql.contains("order")){
			newSql = sql.substring(0, sql.lastIndexOf("order"));
			String orderSQL =  sql.substring(sql.lastIndexOf("order"), sql.length());
			StringBuffer sb = new StringBuffer();
			if(orderSQL.split(" ")[2].contains(".")){
				sb.append(orderSQL.split(" ")[2].substring(orderSQL.split(" ")[2].indexOf(".")+1,orderSQL.split(" ")[2].length()));
			}else{
				sb.append(orderSQL.split(" ")[2]);
			}
			if(orderSQL.contains("desc") || orderSQL.contains("asc")){
				parameter = sb.toString()+" "+orderSQL.split(" ")[3];
			}else{
				parameter = sb.toString()+" asc";
			}
		}
		StringBuffer sqlBuffer = new StringBuffer(newSql);
		sqlBuffer.insert(0, "select * from (select row_number() over(order by page_table_201608120000."+parameter+") rownum,page_table_201608120000.* from (");
		sqlBuffer.append(") page_table_201608120000) page_table_201608120000 where page_table_201608120000.rownum <");
		sqlBuffer.append(offset + b);
		sqlBuffer.append(" and page_table_201608120000.rownum >= ");
		sqlBuffer.append(offset);

		/*
		 * 上面的SQL语句拼接之后大概是这个样子： select * from (
		 * select row_number() over(order by page_table_201608120000.modelrunid desc) rownum,page_table_201608120000.* from (
		 * select * from vi_querydataBMS) page_table_201608120000
		 * ) page_table_201608120000 where page_table_201608120000.rownum <34 and page_table_201608120000.rownum >= 24
		 */
	...
	private String getCountSql(String sql) {
		StringBuffer sqlBuffer = new StringBuffer(sql.contains("order")?sql.substring(0, sql.lastIndexOf("order")):sql);
		// 计算第一条记录的位置，Oracle分页是通过ROWNUM进行的，而ROWNUM是从1开始的
		// 组织SQL语句。
		sqlBuffer.insert(0, "select count(*) from (").append(") page_table_201608120000");
		return sqlBuffer.toString();
	}
}
